...
function Stack.new()
	return setmetatable({
		...
		-- events
		ItemAdded = Loader.Signal(),
		ItemRemoved = Loader.Signal(),
	}, Stack)
end

function Stack:IsEmpty()
	assert(self:IsActive() == true, SETTINGS.Exception.InactiveStackException)
...
function Stack:Pop()
	assert(self:IsActive() == true, SETTINGS.Exception.InactiveStackException)
	assert(self:IsEmpty() == false, SETTINGS.Exception.EmptyStackException)
